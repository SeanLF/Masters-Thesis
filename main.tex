...
A major issue with supervised and semi-supervised learning of data streams is late-arriving or missing class labels. Assuming that properly labelled data will always be available and timely is unreasonable, and, as such, supervised methods are not usually applicable in the real world. Therefore, real-world problems usually require the use of semi-supervised or unsupervised learning techniques. In an intrusion detection problem, it is unreasonable to assume that we will know with utmost certainty whether a network packet is malicious or not before any subsequent packet is processed. Additionally, in semi-supervised learning, "the instances having the highest [predictive] confidence are not necessarily the most useful ones" \cite{homayoun2016review}. We want to determine if self-training can perform well without being selective in an online setting.

This leads us to our contributions. We extend an existing concept drift detector to operate without any labelled data, by using a sliding window of our ensemble's prediction confidence, instead of a boolean indicating whether the ensemble predicted correctly. We also extend selective self-training by using all predictions, and not only those with high predictive confidence. Finally, we developed a novel windowing type for ensembles, as sliding windows are very time consuming and regular tumbling windows are not a suitable replacement. Our windowing technique can be considered a hybrid of the two: we train each sub-classifier in the ensemble with tumbling windows, but delay training in such a way that only one sub-classifier can update its model per iteration.
...
That being said, more research is needed to further reduce the quantity of labelled data used for training, while also increasing its predictive accuracy.
\end{abstract}
...
The research conducted for this thesis was financed by the Province of Ontario and the University of Ottawa.

I would like to thank my supervisor, Dr. Herna Viktor, for her guidance, incredible patience, and, her support.

Finally, I could not have completed this thesis without the support, and encouragement of my friends and family.

Many thanks to each and every one of you.\ldots
...
\include{Appendices/Chapter7Appendix}
